...
disp('Matthew C. Canver 2016')
disp('Send bugs or suggestions Matthew_Canver AT hms DOT harvard DOT com')
disp(' ')
...
reference_seq_title_file = 'filename.fasta';
...
ref_sequence_info = 'filename.bed';
...
perform_wgs_analysis = 0; % enter 1 to turn on WGS analysis or enter 0 to turn off WGS analysis. Must also enter 1 for 'perform_variant_analysis'
...
output_to_excel = 1; % enter 1 to output libraries to excel or enter 0 to skip outputting libraries
...
if perform_haplotype_analysis == 1 | perform_variant_analysis == 1
...
if four_column_bed == 1 && three_column_bed == 0
    ...
    for n = 1:(length(sequence_coordinates)/4)
        ...
        ref_seq_info.identifier(n,1) = sequence_coordinates(row4,1);
        row1 = row1 + 4;
        row2 = row2 + 4;
        row3 = row3 + 4;
        row4 = row4 + 4;
...
if perform_variant_analysis == 1 && end_number > starting_number || perform_variant_analysis == 1 && perform_batch_output == 1
...
for a = starting_number:end_number
    ...
    for b = 1:length(PAM_sequence_list)
        ...
        if perform_variant_analysis == 1
            ...
            row_del = 1;
            row_ins = 1;
            for n = 1:length(vcf_snp.bpstart)
                    ...
                    if length(near_variant_index) == 2 & exist('near_variants','var') == 1 | length(near_variant_index) == 3 & exist('near_variants','var') == 1
                        ...
                        for y = 1:num_iterations
                            ...
                            if length(near_variant_index) == 3 & y > 1
                                ...
                                near_variants.gmaf = near_variants_holder.gmaf(mult_var_guides_index(:,(y-1)));
        ...
        if perform_haplotype_analysis == 0 && perform_variant_analysis == 0 && exist('original_guides_full','var') == 1;
        ...
        if perform_variant_analysis == 1
            ...
            if perform_variant_analysis == 1;
                ...
                unique_variant_guides_full.pam_create(1:length(variant_exact_match.index),1) = {'Variant sgRNA'};
                if length(find(variant_exact_match.index>0))>0
                    unique_variant_guides_full.pam_create(find(variant_exact_match.index>0)) = {'PAM Creation/Alteration'};
                end
                
                if length(find(variant_exact_match.index>0))>0
                    row = 1;
                    for n = 1:length(variant_exact_match_list.index)
                        if five_prime_PAM == 0 && variant_exact_match_list.strand(n,1) == 1 && variant_exact_match_list.index(n,1) > 0 && (variant_exact_match_list.index(n,1)+sgRNA_len+PAM_len-1)<=length(reference_seq)
                        ...
                        elseif five_prime_PAM == 1 && variant_exact_match_list.strand(n,1) == 1 && (variant_exact_match_list.index(n,1)-PAM_len)>0 && (variant_exact_match_list.index(n,1)+sgRNA_len-1)<=length(reference_seq)
                            ...
                            variant_guide_exact_dsb_coord(row,1) = reference_sequence.coord(variant_guide_exact_dsb_index(row,1));
                            variant_guide_exact_strand(row,1) = '+';
                        elseif five_prime_PAM == 0 && variant_exact_match_list.strand(n,1) == 2 && (variant_exact_match_list.index(n,1)-PAM_len)>0 && (variant_exact_match_list.index(n,1)+sgRNA_len-1)<=length(reference_seq)
                            variant_guide_exact_coord_start(row,1) = reference_sequence.coord(variant_exact_match_list.index(n,1))-PAM_len;
                            ...
                            variant_guide_exact_dsb_coord(row,1) = reference_sequence.coord(variant_guide_exact_dsb_index(row,1));
                            variant_guide_exact_strand(row,1) = '-';
                        elseif five_prime_PAM == 1 && variant_exact_match_list.strand(n,1) == 2 && variant_exact_match_list.index(n,1) > 0 && (variant_exact_match_list.index(n,1)+sgRNA_len+PAM_len-1)<=length(reference_seq)
                            variant_guide_exact_coord_start(row,1) = reference_sequence.coord(variant_exact_match_list.index(n,1));
            ...
            chr_string = 'chr';
            for m = 1:length(unique_variant_guides_full.guides)
        ...
        if exist('original_guides_full') == 1
        ...
        if end_number > starting_number && perform_batch_output == 1 && b == length(PAM_sequence_list) && exist('merge_original_guides_full')==1
        ...
        if perform_variant_analysis == 1 && exist('unique_variant_guides_full','var')==1
            ...
            if b == length(PAM_sequence_list)
                ...
                if length(num_spots_batch_temp) == 1
                    num_spots_batch_temp = 0;
                elseif length(num_spots_batch_temp)>1
                    num_spots_batch_temp = sum(num_spots_batch_temp);
                end
            end
        end
        if end_number > starting_number && perform_batch_output == 1 && perform_variant_analysis == 1 && exist('merge_unique_variant_guides_full','var')==1 && b == length(PAM_sequence_list)
            ...
            elseif num_iterate > max(num_associated_variants_batch_temp)
                for j = 1:max(num_associated_variants_batch(a,1))
                    eval(['batch_unique_variant_guides_full',num2str(j),'.bpstart_var((num_spots_batch_temp+1):sum(num_spots_batch),1) = merge_unique_variant_guides_full',num2str(h),'.bpstart_var;']);
                    eval(['batch_unique_variant_guides_full',num2str(j),'.bpend_var((num_spots_batch_temp+1):sum(num_spots_batch),1) = merge_unique_variant_guides_full',num2str(h),'.bpend_var;']);
                    eval(['batch_unique_variant_guides_full',num2str(j),'.rs_id_var((num_spots_batch_temp+1):sum(num_spots_batch),1) = merge_unique_variant_guides_full',num2str(h),'.rs_id_var;']);
                    eval(['batch_unique_variant_guides_full',num2str(j),'.ref_snp_var((num_spots_batch_temp+1):sum(num_spots_batch),1) = merge_unique_variant_guides_full',num2str(h),'.ref_snp_var;']);
                    eval(['batch_unique_variant_guides_full',num2str(j),'.alt_snp_var((num_spots_batch_temp+1):sum(num_spots_batch),1) = merge_unique_variant_guides_full',num2str(h),'.alt_snp_var;']);
                    eval(['batch_unique_variant_guides_full',num2str(j),'.identifier_var((num_spots_batch_temp+1):sum(num_spots_batch),1) = merge_unique_variant_guides_full',num2str(h),'.identifier_var;']);
                    eval(['batch_unique_variant_guides_full',num2str(j),'.variant_index_start_var((num_spots_batch_temp+1):sum(num_spots_batch),1) = merge_unique_variant_guides_full',num2str(h),'.variant_index_start_var;']);
                    eval(['batch_unique_variant_guides_full',num2str(j),'.variant_index_end_var((num_spots_batch_temp+1):sum(num_spots_batch),1) = merge_unique_variant_guides_full',num2str(h),'.variant_index_end_var;']);
                    eval(['batch_unique_variant_guides_full',num2str(j),'.chr_id_var((num_spots_batch_temp+1):sum(num_spots_batch),1) = merge_unique_variant_guides_full',num2str(h),'.chr_id_var;']);
                end
            elseif num_iterate == max(num_associated_variants_batch_temp)
                for h = 1:num_iterate;
                    eval(['batch_unique_variant_guides_full',num2str(h),'.bpstart_var = [batch_unique_variant_guides_full',num2str(h),'.bpstart_var;merge_unique_variant_guides_full',num2str(h),'.bpstart_var];']);
        ...
        if end_number > starting_number && perform_batch_output == 1 && perform_haplotype_analysis == 1 && exist('merge_unique_haplotype_guides_full','var')==1 && b == length(PAM_sequence_list)
            ...
            elseif num_iterate > max(num_associated_variants_batch_temp)
                for j = 1:max(num_associated_variants_batch(a,1))
                    eval(['batch_unique_haplotype_guides_full',num2str(j),'.bpstart_var((num_spots_batch_temp+1):sum(num_spots_batch),1) = merge_unique_haplotype_guides_full',num2str(h),'.bpstart_var;']);
                    eval(['batch_unique_haplotype_guides_full',num2str(j),'.bpend_var((num_spots_batch_temp+1):sum(num_spots_batch),1) = merge_unique_haplotype_guides_full',num2str(h),'.bpend_var;']);
                    eval(['batch_unique_haplotype_guides_full',num2str(j),'.rs_id_var((num_spots_batch_temp+1):sum(num_spots_batch),1) = merge_unique_haplotype_guides_full',num2str(h),'.rs_id_var;']);
                    eval(['batch_unique_haplotype_guides_full',num2str(j),'.ref_snp_var((num_spots_batch_temp+1):sum(num_spots_batch),1) = merge_unique_haplotype_guides_full',num2str(h),'.ref_snp_var;']);
                    eval(['batch_unique_haplotype_guides_full',num2str(j),'.alt_snp_var((num_spots_batch_temp+1):sum(num_spots_batch),1) = merge_unique_haplotype_guides_full',num2str(h),'.alt_snp_var;']);
                    eval(['batch_unique_haplotype_guides_full',num2str(j),'.identifier_var((num_spots_batch_temp+1):sum(num_spots_batch),1) = merge_unique_haplotype_guides_full',num2str(h),'.identifier_var;']);
                    eval(['batch_unique_haplotype_guides_full',num2str(j),'.variant_index_start_var((num_spots_batch_temp+1):sum(num_spots_batch),1) = merge_unique_haplotype_guides_full',num2str(h),'.variant_index_start_var;']);
                    eval(['batch_unique_haplotype_guides_full',num2str(j),'.variant_index_end_var((num_spots_batch_temp+1):sum(num_spots_batch),1) = merge_unique_haplotype_guides_full',num2str(h),'.variant_index_end_var;']);
                    eval(['batch_unique_haplotype_guides_full',num2str(j),'.chr_id_var((num_spots_batch_temp+1):sum(num_spots_batch),1) = merge_unique_haplotype_guides_full',num2str(h),'.chr_id_var;']);
                end
            elseif num_iterate == max(num_associated_variants_batch_temp)
                for h = 1:num_iterate;
                    eval(['batch_unique_haplotype_guides_full',num2str(h),'.bpstart_var = [batch_unique_haplotype_guides_full',num2str(h),'.bpstart_var;merge_unique_haplotype_guides_full',num2str(h),'.bpstart_var];']);
        ...
        if perform_variant_analysis == 1 && end_number == starting_number && output_to_excel == 1 && b==length(PAM_sequence_list) || perform_variant_analysis == 1 && individual_output == 1 && output_to_excel == 1 && b==length(PAM_sequence_list);
        ...
        if perform_variant_analysis == 1 && end_number == starting_number && b==length(PAM_sequence_list) && output_to_excel == 0 && exist('merge_unique_variant_guides_full','var')==1 || perform_variant_analysis == 1 && individual_output == 1 && output_to_excel == 0 && b==length(PAM_sequence_list) && exist('merge_unique_variant_guides_full','var')==1;
        ...
        if perform_plot_analysis == 1 && end_number == starting_number && b==length(PAM_sequence_list) || perform_plot_analysis == 1 && individual_output == 1 && b==length(PAM_sequence_list)
            ...
            if perform_haplotype_analysis == 0 && perform_variant_analysis == 0
...
if perform_variant_analysis == 1 && end_number > starting_number && output_to_excel == 0 && perform_batch_output == 1
    array_cell_variant = {'ID' 'PAM' 'sgRNA_Sequence' 'sgRNA_PAM' 'Full_Sequence' 'Strand' 'Chromosome' 'Classification' 'N_W_Alignment_Score' 'No_Variant_ID' 'No_Variant_sgRNA_Sequence' 'No_Variant_PAM_Only' 'No_Variant_Full_Sequence' 'No_Variant_Strand' 'No_Variant_Chromosome' 'Guide_Start_Coordinate' 'Guide_End_Coordinate' 'DSB_Coordinate' 'Guide_Start_Index' 'Guide_End_Index' 'DSB_Index' 'Filename'};
...
if end_number > starting_number && perform_batch_output == 1 && perform_haplotype_analysis == 1 && output_to_excel == 1
    ...
    if end_number > starting_number && perform_batch_output == 1 && perform_haplotype_analysis == 1 && output_to_excel == 1
        num_iterate = max(num_associated_variants);
    end
    for h = 1:num_iterate;
...
if end_number > starting_number && perform_batch_output == 1 && perform_variant_analysis == 1 && output_to_excel == 1
